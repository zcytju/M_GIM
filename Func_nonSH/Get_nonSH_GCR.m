function [G_R, G_S, C_R, C_S, R_R, R_S, IONC, m0, NN] = Get_nonSH_GCR(fig,doy ,Sites_Info,sate,SDCB_REF,K,M,PG,PC,PR,sate_mark)
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,C_n_r,R_n_r,G_n_s,C_n_s,R_n_s,i,K,M);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:C_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_BDSMatrix(fig,BDSP4,bdsx,bdsy,bdsz,sx,sy,sz,G_n_r,C_n_r,R_n_r,G_n_s,C_n_s,R_n_s,i,K,M);
    N=N+sN'*sN*PC;
    U=U+sN'*sl*PC;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:R_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GLOMatrix(fig,GLOP4,glox,gloy,gloz,sx,sy,sz,G_n_r,C_n_r,R_n_r,G_n_s,C_n_s,R_n_s,i,K,M);
    N=N+sN'*sN*PR;
    U=U+sN'*sl*PR;
    %--RMS
    sizel=sizel+length(sl);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,bds_n_r,glo_n_r,gps_n_s,bds_n_s,glo_n_s,ith,K,M)
MC=[];l=[];
num=(K+1)^2-(K-M)*(K-M+1);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_BDSMatrix(fig,BDSP4,x,y,z,sx,sy,sz,gps_n_r,bds_n_r,glo_n_r,gps_n_s,bds_n_s,glo_n_s,ith,K,M)
MC=[];l=[];
num=(K+1)^2-(K-M)*(K-M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
figt=2880/fig;
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GLOMatrix(fig,GLOP4,x,y,z,sx,sy,sz,gps_n_r,bds_n_r,glo_n_r,gps_n_s,bds_n_s,glo_n_s,ith,K,M)
MC=[];l=[];
