function [G_R, G_S, E_R, E_S, EX_R, EX_S, IONC, m0, NN] = Get_POLY_GE(fig,doy ,Sites_Info,sate,SDCB_REF,K,M,lat0,lon0,PG,PE,sate_mark)
...
%--LS estimate
n_m=((K+1)*(M+1))*fig;
num=n_m+G_n_s+G_n_r+E_n_s+E_n_r+EX_n_s+EX_n_r;
N=zeros(num,num);
U=zeros(num,1);
L=0; sizel=0;
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,G_n_s,E_n_s,EX_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:E_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALMatrix(fig,GALP4,galx,galy,galz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,G_n_s,E_n_s,EX_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:EX_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALXMatrix(fig,GALXP4,galxx,galxy,galxz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,G_n_s,E_n_s,EX_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,gps_n_s,gal_n_s,galx_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GALMatrix(fig,GALP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,gps_n_s,gal_n_s,galx_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
figt=2880/fig;
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GALXMatrix(fig,GALXP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,gps_n_s,gal_n_s,galx_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
figt=2880/fig;
