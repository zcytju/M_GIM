function [G_R, G_S, E_R, E_S, EX_R, EX_S, C_R, C_S, R_R, R_S, IONC, m0, NN] = Get_POLY_GCER(fig,doy ,Sites_Info,sate,SDCB_REF,K,M,lat0,lon0,PG,PC,PE,PR,sate_mark)
...
%     PG,PC,PE,PR: weight of different system observations
% OUTPUT:
%     G_R, G_S, E_R, E_S, EX_R, EX_S, C_R, C_S, R_R, R_S: estimated receiver and satellite DCBs
...
%--LS estimate
n_m=((K+1)*(M+1))*fig;
num=n_m+G_n_s+G_n_r+E_n_s+E_n_r+EX_n_s+EX_n_r+C_n_s+C_n_r+R_n_s+R_n_r;
N=zeros(num,num);
U=zeros(num,1);
L=0; sizel=0;
...
C_BDS(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+1:G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s)=ones(1,C_n_s);
C_GLO(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+1:G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+R_n_s)=ones(1,R_n_s);
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:E_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALMatrix(fig,GALP4,galx,galy,galz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:EX_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALXMatrix(fig,GALXP4,galxx,galxy,galxz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:C_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_BDSMatrix(fig,BDSP4,bdsx,bdsy,bdsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PC;
    U=U+sN'*sl*PC;
    %--RMS
    sizel=sizel+length(sl);
    ...
    %-----------
    clear BDSP4;
    disp(['3.----- [ ',num2str(i),' / ',num2str(C_n_r),' ] ',num2str(i/C_n_r*100),'% BDS data has constructed !']);
end
...
for i=1:R_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GLOMatrix(fig,GLOP4,glox,gloy,gloz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PR;
    U=U+sN'*sl*PR;
    %--RMS
    sizel=sizel+length(sl);
    ...
    %-----------
    clear GLOP4;
    disp(['4.----- [ ',num2str(i),' / ',num2str(R_n_r),' ] ',num2str(i/R_n_r*100),'% GLONASS data has constructed !']);
end

N=N+C_GPS'*C_GPS+C_GAL'*C_GAL+C_GALX'*C_GALX+C_BDS'*C_BDS+C_GLO'*C_GLO;
U=U+C_GPS'*G_Wx+C_GAL'*E_Wx+C_GALX'*EX_Wx+C_BDS'*C_Wx+C_GLO'*R_Wx;
L=L+G_Wx'*G_Wx+E_Wx'*E_Wx+EX_Wx'*EX_Wx+C_Wx'*C_Wx+R_Wx'*R_Wx;
R=pinv(N)*U;
...
C_R=R(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+1:G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r)*10^9/299792458;
temp_bds=linspace(1,bdsnum,bdsnum);
temp_bds(bds_d_sat)=[];
C_S(temp_bds)=R(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+1:G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s)*10^9/299792458;

R_R=R(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+1:G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r)*10^9/299792458;
temp_glo=linspace(1,glonum,glonum);
temp_glo(glo_d_sat)=[];
R_S(temp_glo)=R(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+1:G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+R_n_s)*10^9/299792458;

IONC=R(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+R_n_s+1:end);
%--RMS
V=L-R'*U;
f=sizel-num;
m0=sqrt(V/f);
NN=N(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+R_n_s+1:end,G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+R_n_r+R_n_s+1:end);
%------
end

%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,glo_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,glo_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
global sample_num;
num=(K+1)*(M+1);
...
for i=1:fig
    for j=1:gps_n_s                %-----------------------j is satellite number
        parfor k=figt*i-(figt-1):figt*i %-------------------------k is epoch number
            ...
            M_col=linspace(0,0,num*fig+gps_n_s+gps_n_r+gal_n_s+gal_n_r+galx_n_s+galx_n_r+bds_n_s+bds_n_r+glo_n_s+glo_n_r);
            ...
            st=num*(i-1)+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+glo_n_s+1;
            ed=num*i+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+glo_n_s;
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GALMatrix(fig,GALP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,glo_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,glo_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
global sample_num;
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GALXMatrix(fig,GALXP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,glo_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,glo_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
global sample_num;
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_BDSMatrix(fig,BDSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,glo_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,glo_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
global sample_num;
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
...
for i=1:fig
    for j=1:bds_n_s                %-----------------------j is satellite number
        parfor k=figt*i-(figt-1):figt*i %-------------------------k is epoch number
            ...
            t_r=30*(k-1)*pi/43200;
            [b,s]=Get_IPP(E,A,sb,sl,IPPz,t_r);%
            M_col(gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+ith)=(-8.99768938)*cos(IPPz);   %-----station dcb coefficient
            M_col(gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+j)=(-8.99768938)*cos(IPPz); %---satallite dcb coefficient
            st=num*(i-1)+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+glo_n_s+1;
            ed=num*i+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+glo_n_s;
            M_col(st:ed)=Get_poly_coef(b,s,K,M,lat0,lon0);
            M_scol=sparse(M_col);
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GLOMatrix(fig,GLOP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,glo_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,glo_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
global sample_num;
...
for i=1:fig
    for j=1:glo_n_s                %-----------------------j is satellite number
        parfor k=figt*i-(figt-1):figt*i %-------------------------k is epoch number
            ...
            t_r=30*(k-1)*pi/43200;
            [b,s]=Get_IPP(E,A,sb,sl,IPPz,t_r);%
            M_col(gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+ith)=R(j)*cos(IPPz);   %-----station dcb coefficient
            M_col(gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+j)=R(j)*cos(IPPz); %---satallite dcb coefficient
            st=num*(i-1)+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+glo_n_s+1;
            ed=num*i+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+glo_n_r+glo_n_s;
            M_col(st:ed)=Get_poly_coef(b,s,K,M,lat0,lon0);
            M_scol=sparse(M_col);
