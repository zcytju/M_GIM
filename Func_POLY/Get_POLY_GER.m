function [G_R, G_S, E_R, E_S, EX_R, EX_S, R_R, R_S, IONC, m0, NN] = Get_POLY_GER(fig,doy ,Sites_Info,sate,SDCB_REF,K,M,lat0,lon0,PG,PE,PR,sate_mark)
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:E_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALMatrix(fig,GALP4,galx,galy,galz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:EX_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALXMatrix(fig,GALXP4,galxx,galxy,galxz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:R_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GLOMatrix(fig,GLOP4,glox,gloy,gloz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,R_n_r,G_n_s,E_n_s,EX_n_s,R_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PR;
    U=U+sN'*sl*PR;
    %--RMS
    sizel=sizel+length(sl);
