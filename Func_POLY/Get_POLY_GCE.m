function [G_R, G_S, E_R, E_S, EX_R, EX_S, C_R, C_S, IONC, m0, NN] = Get_POLY_GCE(fig,doy ,Sites_Info,sate,SDCB_REF,K,M,lat0,lon0,PG,PC,PE,sate_mark)
...
%     PG,PC,PE: weight of different system observations
% OUTPUT:
%     G_R, G_S, E_R, E_S, EX_R, EX_S, C_R, C_S: estimated receiver and satellite DCBs
...
%--LS estimate
n_m=((K+1)*(M+1))*fig;
num=n_m+G_n_s+G_n_r+E_n_s+E_n_r+EX_n_s+EX_n_r+C_n_s+C_n_r;
N=zeros(num,num);
U=zeros(num,1);
L=0; sizel=0;
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
    %--RMS
...
for i=1:E_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALMatrix(fig,GALP4,galx,galy,galz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:EX_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALXMatrix(fig,GALXP4,galxx,galxy,galxz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    %--RMS
    sizel=sizel+length(sl);
...
for i=1:C_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_BDSMatrix(fig,BDSP4,bdsx,bdsy,bdsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,K,M,lat0,lon0);
    N=N+sN'*sN*PC;
    U=U+sN'*sl*PC;
    %--RMS
    sizel=sizel+length(sl);
...
N=N+C_GPS'*C_GPS+C_GAL'*C_GAL+C_GALX'*C_GALX+C_BDS'*C_BDS;
U=U+C_GPS'*G_Wx+C_GAL'*E_Wx+C_GALX'*EX_Wx+C_BDS'*C_Wx;
L=L+G_Wx'*G_Wx+E_Wx'*E_Wx+EX_Wx'*EX_Wx+C_Wx'*C_Wx;
R=pinv(N)*U;
...
IONC=R(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+1:end);
%--RMS
V=L-R'*U;
f=sizel-num;
m0=sqrt(V/f);
NN=N(G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+1:end,G_n_r+G_n_s+E_n_r+E_n_s+EX_n_r+EX_n_s+C_n_r+C_n_s+1:end);
%------
end

%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
...
for i=1:fig
    for j=1:gps_n_s                %-----------------------j is satellite number
        parfor k=figt*i-(figt-1):figt*i %-------------------------k is epoch number
            ...
            M_col=linspace(0,0,num*fig+gps_n_s+gps_n_r+gal_n_s+gal_n_r+galx_n_s+galx_n_r+bds_n_s+bds_n_r);
            ...
            st=num*(i-1)+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s+1;
            ed=num*i+gps_n_r+gps_n_s+gal_n_r+gal_n_s+galx_n_r+galx_n_s+bds_n_r+bds_n_s;
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GALMatrix(fig,GALP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
figt=2880/fig;
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_GALXMatrix(fig,GALXP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
figt=2880/fig;
...
%% ------------------------------sub_function--------------------------------
function [MC,l]=Get_BDSMatrix(fig,BDSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,K,M,lat0,lon0)
MC=[];l=[];
num=(K+1)*(M+1);
[sb,sl]=XYZtoBLH(sx,sy,sz);
figt=2880/fig;
