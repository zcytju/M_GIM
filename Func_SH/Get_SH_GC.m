function [G_R, G_S, C_R, C_S, IONC, m0, NN] = Get_SH_GC(fig,doy ,Sites_Info,sate,SDCB_REF,order,PG,PC,sate_mark)
...
%--LS estimate
num=(order+1)^2*fig+G_n_s+G_n_r+C_n_s+C_n_r;
N=zeros(num,num);
U=zeros(num,1);
L=0; sizel=0;
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,C_n_r,G_n_s,C_n_s,i,order);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
...
for i=1:C_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_BDSMatrix(fig,BDSP4,bdsx,bdsy,bdsz,sx,sy,sz,G_n_r,C_n_r,G_n_s,C_n_s,i,order);
    N=N+sN'*sN*PC;
    U=U+sN'*sl*PC;
    sizel=sizel+length(sl);
    L=L+sl'*sl*PC;
