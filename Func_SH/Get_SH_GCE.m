function [G_R, G_S, E_R, E_S, EX_R, EX_S, C_R, C_S, IONC, m0, NN] = Get_SH_GCE(fig,doy ,Sites_Info,sate,SDCB_REF,order,PG,PC,PE,sate_mark)
...
%--LS estimate
num=(order+1)^2*fig+G_n_s+G_n_r+E_n_s+E_n_r+EX_n_s+EX_n_r+C_n_s+C_n_r;
N=zeros(num,num);
U=zeros(num,1);
L=0; sizel=0;
...
for i=1:G_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GPSMatrix(fig,GPSP4,gpsx,gpsy,gpsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,order);
    N=N+sN'*sN*PG;
    U=U+sN'*sl*PG;
...
for i=1:E_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALMatrix(fig,GALP4,galx,galy,galz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,order);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    sizel=sizel+length(sl);
    L=L+sl'*sl*PE;
...
for i=1:EX_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_GALXMatrix(fig,GALXP4,galxx,galxy,galxz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,order);
    N=N+sN'*sN*PE;
    U=U+sN'*sl*PE;
    sizel=sizel+length(sl);
    L=L+sl'*sl*PE;
...
for i=1:C_n_r
    ...
    sy=Coor(index,2);
    sz=Coor(index,3);
    [sN,sl]=Get_BDSMatrix(fig,BDSP4,bdsx,bdsy,bdsz,sx,sy,sz,G_n_r,E_n_r,EX_n_r,C_n_r,G_n_s,E_n_s,EX_n_s,C_n_s,i,order);
    N=N+sN'*sN*PC;
    U=U+sN'*sl*PC;
    sizel=sizel+length(sl);
    L=L+sl'*sl*PC;
...
%% ------------------------------sub_function--------------------------------
function [M,l]=Get_GPSMatrix(fig,GPSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,order)
M=[];
l=[];
global sample_num;
num=(order+1)^2;
...
%% ------------------------------sub_function--------------------------------
function [M,l]=Get_GALMatrix(fig,GALP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,order)
M=[];
l=[];
global sample_num;
num=(order+1)^2;
...
%% ------------------------------sub_function--------------------------------
function [M,l]=Get_GALXMatrix(fig,GALXP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,order)
M=[];
l=[];
global sample_num;
num=(order+1)^2;
...
%% ------------------------------sub_function--------------------------------
function [M,l]=Get_BDSMatrix(fig,BDSP4,x,y,z,sx,sy,sz,gps_n_r,gal_n_r,galx_n_r,bds_n_r,gps_n_s,gal_n_s,galx_n_s,bds_n_s,ith,order)
M=[];
l=[];
global sample_num;
num=(order+1)^2;
